...
def sample_c(m, n, ind=-1):
	...
	for i in range(m):
		...
		c[i,0] = np.random.randint(10)
		c[i,1]=np.random.uniform(-2.,2.)
		c[i,2]=np.random.uniform(-3.,3.)
	return c
...
class InfoGAN():
	def __init__(self, generator, discriminator, data):
		...
		self.c_dim = 3 # expanded condition to 3 ,each represents diffrent class
	...
	def train(self, sample_dir, ckpt_dir='ckpt', training_epoches = 1000000, batch_size = 64):
		...
		for epoch in range(training_epoches):
			...
			if epoch % 100 == 0 or epoch < 100:
				...
				if epoch % 2000 == 0:
					self.saver.save(self.sess, os.path.join(ckpt_dir, "infogan.expanded2.ckpt"))
...
if __name__ == '__main__':
	...
	sample_dir = 'Samples/mnist_infogan_conv_expanded2'
	if not os.path.exists(sample_dir):
		os.makedirs(sample_dir)

	# param
	generator = G_conv_mnist()
	discriminator = D_conv_mnist_expanded()
	data = mnist()
