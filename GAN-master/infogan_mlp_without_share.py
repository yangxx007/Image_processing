...
class InfoGAN():
	def __init__(self, generator, discriminator, classifier, data):
		...
		self.Q_solver = tf.train.AdamOptimizer().minimize(self.Q_loss, var_list=self.generator.vars + self.classifier.vars)
	...
	def train(self, sample_dir, ckpt_dir='ckpt', training_epoches = 1000000, batch_size = 64):
		...
		for epoch in range(training_epoches):
			...
			if epoch % 100 == 0 or epoch < 100:
				...
				if epoch % 2000 == 0:
					self.saver.save(self.sess, os.path.join(ckpt_dir, "infogan.ckpt-"+str(epoch))	)
...
if __name__ == '__main__':
	...
	sample_dir = 'Samples/mnist_infogan_mlp_without_share'
